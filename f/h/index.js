(function() {
  ...
  let text = `Yellow: long sentence
Red: complicated
Purple: long word. Mouse over.
Blue: adverb, weak phrases 
Green: passive voice.`;
  ...
  function counters() {
    document.querySelector("#adverb").innerHTML = `Adverb: ${
      data.adverbs
    }${data.adverbs > 1 ? "" : ""}. Use ${Math.round(
      data.paragraphs / 3
    )}`;
    document.querySelector(
      "#passive"
    ).innerHTML = `Passive voice: ${data.passiveVoice}${
      data.passiveVoice > 1 ? "" : ""
    }${Math.round(data.sentences / 5)}`;
    document.querySelector("#complex").innerHTML = `Complex phrase: ${data.complex}${
      data.complex > 1 ? "" : ""
    }`;
    document.querySelector("#hardSentence").innerHTML = `Hard to read: ${
      data.hardSentences
    } of ${data.sentences}${
      data.sentences > 1 ? "" : ""
    }`;
    document.querySelector("#veryHardSentence").innerHTML = `Very hard to read: ${
